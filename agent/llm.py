...
def call_anthropic(system_prompt: str, prompt: str, images: List[Path]) -> str:
    ...
    response = anthropic_client.messages.create(model=ANTHROPIC_MODEL, max_tokens=8192, system=system_prompt, messages=messages)
...
def call_openai(system_prompt: str, prompt: str, images: List[Path]) -> str:
    ...
    response = openai_client.chat.completions.create(model=OPENAI_MODEL, max_tokens=8192, messages=[{"role": "system", "content": system_prompt}, *messages], response_format={"type": "json_object"})
    return response.choices[0].message.content

def call_groq(system_prompt: str, prompt: str) -> str:
    response = groq_client.chat.completions.create(model=GROQ_MODEL, max_tokens=8192, messages=[{"role": "system", "content": system_prompt}, {"role": "user", "content": prompt}], response_format={"type": "json_object"})
    return response.choices[0].message.content

def extract_json_from_response(text: str) -> Dict:
    # --- ROBUSTNESS FIX: Aggressive JSON cleaning for Anthropic provider ---
    # Find the start of the JSON object
    start_brace_index = text.find('{')
    if start_brace_index == -1:
        raise ValueError(f"No JSON object found in the model's response: {text}")

    # Find the end of the JSON object, even if it's not the last character
    end_brace_index = text.rfind('}')
    if end_brace_index == -1:
        # If no closing brace, assume it's truncated and take the rest of the string
        json_str = text[start_brace_index:]
    else:
        # Extract the string from the first '{' to the last '}'
        json_str = text[start_brace_index : end_brace_index + 1]

    # Attempt to fix common JSON errors
    try:
        # Clean trailing commas before brackets and braces
        json_str_fixed = re.sub(r',\s*([\}\]])', r'\1', json_str)
        
        # Balance braces to fix truncation
        ...
        raise ValueError(f"Failed to decode JSON after cleaning: {e}\nOriginal string was: {json_str}")
